...
public interface ReplyRepository extends JpaRepository<Reply,Long> {
    ...
    Page<Object[]> getReplyWithReBoard(@Param("bno")Long bno, Pageable pageable);
    
    @EntityGraph(attributePaths = {"replyer","board"})
    @Query(value = "select r from Reply r where r.board.bno =:bno "
    , countQuery = "select count(r) from Reply r where r.board.bno =:bno")
    Page<Reply> getReplyWithReBoard2(@Param("bno")Long bno, Pageable pageable);
    
    @EntityGraph(attributePaths = "replyer")
    @Query(value ="select r from Reply r where r.board.bno = :bno"
    ,countQuery = "select count(r) from Reply r where r.board.bno =:bno")
    Page<Reply> ex2(@Param("bno") Long bno, Pageable pageable);
}